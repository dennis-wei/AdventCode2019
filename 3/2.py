def parse_move(move_string):
    return (move_string[0], int(move_string[1:]))

def run_simulation(string_move_strings):
    distances = {(0, 0): 0}
    moves = [parse_move(m) for m in string_move_strings.split(",")]
    curr_x = 0
    curr_y = 0
    distance_moved = 0
    reached = set()
    for m in moves:
        if m[0] == "D":
            for i in range(m[1]):
                distance_moved += 1
                curr_y -= 1
                reached.add((curr_x, curr_y))
                if (curr_x, curr_y) not in distances:
                    distances[(curr_x, curr_y)] = distance_moved
        if m[0] == "U":
            for i in range(m[1]):
                distance_moved += 1
                curr_y += 1
